...
export default function SettingsModal({
  ...
  const drawPopupRender = (canvas: HTMLCanvasElement, eff: any, time: number, text: string) => {
    ...
    let bgOffsetX = 0;
    let bgOffsetY = 0;
    const blurInOutDir = eff.blurBgInOutEffect || 'bottom-to-top';
  ...
  if (activeSimTime < 1.0) {
    ...
    const bgDir = config.blurBgInOutEffect || 'bottom-to-top';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md" id="settings-modal-overlay">
      <div 
        className="bg-[#0E0E11] border border-white/10 rounded-2xl w-full max-w-2xl flex flex-col max-h-[90vh] shadow-2xl overflow-hidden animate-in fade-in duration-200"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="p-5 border-b border-white/10 bg-[#131317] flex items-center justify-[space-between] justify-between">
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1 bg-[#0A0A0C]/35">
              ...
              <div className="bg-blue-500/5 p-4 rounded-xl border border-blue-500/10 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-4 items-center">
                  ...
                  <div>
                    ...
                    >
                      <span>{config.enableKenBurns ? "Đang mở (On)" : "Đang tắt (Off)"}</span>
...
{(config.enableTextHighlight || config.subtitleShowEffect === 'highlight_two_words') && (
          ...
          {activeTab === 'bg-music' && (
            <div className="space-y-5 animate-in fade-in duration-200">
              <div className="bg-[#050505]/40 border border-white/10 rounded-xl p-5 space-y-4">
                ...
                {/* Real-time Audition / Mixer Panel */}
                <div className="bg-[#121217] border border-indigo-500/20 rounded-xl p-4 space-y-3.5">
                  <div className="flex items-center justify-between">
                    <h4 className="text-xs font-bold text-white flex items-center gap-2">
                      <span className="p-1 bg-indigo-500/20 text-indigo-400 rounded-md">🔊</span>
                      Xem thử Phối Hợp (Thoại + Nhạc nền)
                    </h4>
                    {isAuditionPlaying && (
                  ...
                  <p className="text-[10px] text-white/40 leading-relaxed">
                    Tính năng phát đồng thời giọng đọc (Voiceover) chính và một bài Nhạc nền để bạn thẩm âm chuẩn xác nhất, giúp tìm ra mức điều chỉnh âm lượng hài hòa.
                  </p>

                  <div className="grid grid-cols-2 gap-3 pt-1">
                    {/* Voiceover Selection Status / Trial Upload */}
                    <div className="bg-[#050505] p-2.5 rounded-lg border border-white/5 flex flex-col justify-between">
                      <div>
                        <span className="text-[9px] uppercase tracking-wider text-white/30 font-bold block mb-1.5">1. File Thuyết Minh</span>
                        {trialVoiceFile ? (
                          <div className="flex items-center justify-between gap-1">
                            <span className="text-[10.5px] text-emerald-400 truncate font-medium flex-1" title={trialVoiceFile.name}>
                              {trialVoiceFile.name}
                            </span>
                            <button
                              type="button"
                              onClick={() => {
                                handleAuditionStop();
                                setTrialVoiceFile(null);
                              }}
                              className="text-[9px] text-rose-400 hover:underline shrink-0 font-bold"
                            >
                              Xóa
                            </button>
                          </div>
                        ) : audioFile ? (
                          <span className="text-[10.5px] text-indigo-400 truncate font-medium block" title={audioFile.name}>
                            🎤 Đã nạp từ dự án chính
                          </span>
                        ) : (
                          <div className="space-y-1.5">
                            <span className="text-[10px] text-white/55 block">Mặc định: chưa có voice dự án.</span>
                            <input
                              type="file"
                              accept="audio/mp3,audio/wav,audio/m4a,audio/ogg"
                              id="trial-voice-upload"
                              className="hidden"
                              onChange={(e) => {
                                const files = e.target.files;
                                if (files && files.length > 0) {
                                  handleAuditionStop();
                                  setTrialVoiceFile(files[0]);
                                }
                              }}
                            />
                            <button
                              type="button"
                              onClick={() => document.getElementById('trial-voice-upload')?.click()}
                              className="text-[10px] text-indigo-400 hover:underline flex items-center gap-1 font-bold"
                            >
                              ➕ Nạp file voice thử nghiệm
                            </button>
                          </div>
                        )}
                      </div>

                      {/* Control original MP3 voice volume */}
                      <div className="mt-3 pt-2.5 border-t border-white/5 space-y-1.5">
                        <div className="flex items-center justify-between text-[9.5px]">
                          <span className="text-white/40">Âm lượng file gốc:</span>
                          <span className="text-indigo-400 font-bold">{config.mainAudioVolume !== undefined ? config.mainAudioVolume : 100}%</span>
                        </div>
                        <input
                          type="range"
                          min="0"
                          max="200"
                          step="5"
                          value={config.mainAudioVolume !== undefined ? config.mainAudioVolume : 100}
                    ...
                    {/* Audition Music Selector */}
                    <div className="bg-[#050505] p-2.5 rounded-lg border border-white/5">
                      <span className="text-[9px] uppercase tracking-wider text-white/30 font-bold block mb-1">2. Bài nhạc nền nghe thử</span>
                      {bgMusicFiles.length > 0 ? (
                        <select
                          value={auditionMusicTrackId || (bgMusicFiles[0] ? bgMusicFiles[0].id : '')}
                          onChange={(e) => {
                            handleAuditionStop();
                            setAuditionMusicTrackId(e.target.value);
                          }}
                          className="w-full bg-[#121217] border border-white/10 rounded px-2 py-1 text-[10.5px] text-white/80 focus:outline-none focus:border-indigo-500"
                        >
                          {!auditionMusicTrackId && <option value="">-- Chọn bài nhạc --</option>}
                          {bgMusicFiles.map((bg) => (
                            <option key={bg.id} value={bg.id}>
                              {bg.name} ({bg.volume !== undefined ? bg.volume : 100}%)
                            </option>
                          ))}
                        </select>
                      ) : (
                        <span className="text-[10px] text-white/50 block pt-1">Vui lòng tải nhạc nền lên trước.</span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="pt-1">
                    <button
                      ...
                      disabled={bgMusicFiles.length === 0 && !audioFile && !trialVoiceFile}
                      className={`w-full py-2.5 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-2 ${
                        isAuditionPlaying
                          ? 'bg-amber-600 hover:bg-amber-500 text-white shadow-md'
                          : 'bg-indigo-600 hover:bg-indigo-500 text-white disabled:opacity-40 disabled:cursor-not-allowed shadow-md shadow-indigo-900/20'
                      }`}
                    >
                      {isAuditionPlaying ? (
                        <>
                          <Pause size={13} />
                          <span>Dừng nghe thử phối âm</span>
                        </>
                      ) : (
                        <>
                          <Play size={13} />
                          <span>Phát thử phối hợp Voice + Nhạc nền</span>
                        </>
                      )}
                    </button>
                  </div>
                ...
                <div className="hidden">
                        ...
                        {config.enableHighlightDate && (
                          <div className="space-y-4 animate-in fade-in duration-200 pt-2 border-t border-white/5">
                            ...
                            <div className="space-y-1.5">
                              ...
                              >
                                {['Josefin Sans', 'Inter', 'Space Grotesk', 'Outfit', 'Montserrat', 'Playfair Display', 'Oswald', 'Lobster', 'Anton', 'Pacifico'].map((f) => (
                  ...
                  <div className="border border-white/10 rounded-lg overflow-hidden bg-[#0a0a0f]">
                    ...
                    {activeBehaviorAccordion === 10 && (
                      <div className="p-4 border-t border-white/5 bg-[#050505]/60 space-y-4 animate-in slide-in-from-top-2 duration-200">
                        <div className="flex items-center justify-between">
                          <label htmlFor="enableFakeVideoEditor" className="text-[11.5px] text-white/95 font-bold cursor-pointer uppercase tracking-wider">
                            Kích hoạt hiệu ứng FAKE EDIT VIDEO
                          </label>
                          <input
                            type="checkbox"
                            id="enableFakeVideoEditor"
                            checked={!!config.enableFakeVideoEditor}
                            onChange={(e) => updateConfig('enableFakeVideoEditor', e.target.checked)}
                            className="w-4 h-4 rounded border-white/10 text-indigo-500 focus:ring-indigo-500/20 bg-black cursor-pointer"
                          />
                        </div>

                        {config.enableFakeVideoEditor && (
                          <div className="space-y-4 animate-in fade-in duration-200 pt-3 border-t border-white/5">
                            <div className="space-y-1.5">
                  ...
                  <div className="border border-white/10 rounded-lg overflow-hidden bg-[#0a0a0f]">
                    ...
                    {activeBehaviorAccordion === 12 && (
                      <div className="p-4 border-t border-white/5 bg-[#050505]/60 space-y-4 animate-in slide-in-from-top-2 duration-200">
                        <div className="flex items-center justify-between">
                          <label htmlFor="enableTouchTyping" className="text-[11.5px] text-white/95 font-bold cursor-pointer uppercase tracking-wider">
                            Kích hoạt hành vi GÕ CẢM ỨNG
                          </label>
                          <input
                            type="checkbox"
                            id="enableTouchTyping"
                            checked={!!config.enableTouchTyping}
                            onChange={(e) => updateConfig('enableTouchTyping', e.target.checked)}
                            className="w-4 h-4 rounded border-white/10 text-blue-500 focus:ring-blue-500/20 bg-black cursor-pointer"
                          />
                        </div>

                        {config.enableTouchTyping && (
                            ...
                            <div className="pt-3 border-t border-white/5 space-y-3">
                              <div className="flex items-center justify-between">
                                ...
                                <input
                                  ...
                                  className="w-3.5 h-3.5 rounded border-white/10 text-blue-500 focus:ring-blue-500/20 bg-black cursor-pointer"
                                />
                              </div>
                  ...
                  <div className="border border-white/10 rounded-lg overflow-hidden bg-[#0a0a0f]">
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <span className="text-[11.5px] font-bold text-white block">HÀNH VI VẼ VÒNG TRÒN CHUỘT</span>
                          <span className="text-[10px] text-white/40">Con trỏ chuột di chuyển lên ngẫu nhiên, vẽ vòng tròn & mũi tên chỉ điểm tự nhiên</span>
                        </div>
                      </div>
                    ...
                    {activeBehaviorAccordion === 13 && (
                      <div className="p-4 border-t border-white/5 bg-[#050505]/60 space-y-4 animate-in slide-in-from-top-2 duration-200">
                        <div className="flex items-center justify-between">
                          <label htmlFor="enableDrawCircle" className="text-[11.5px] text-white/95 font-bold cursor-pointer uppercase tracking-wider">
                            Kích hoạt hành vi VẼ VÒNG TRÒN CHUỘT
                          </label>
                          <input
                            type="checkbox"
                            id="enableDrawCircle"
                            checked={!!config.enableDrawCircle}
                            onChange={(e) => updateConfig('enableDrawCircle', e.target.checked)}
                            className="w-4 h-4 rounded border-white/10 text-red-500 focus:ring-red-500/20 bg-black cursor-pointer"
                          />
                        </div>

                        {config.enableDrawCircle && (
                          <div className="space-y-4 pt-3 border-t border-white/5 animate-in fade-in duration-200">
                            <div className="space-y-1.5">
                            ...
                            <p className="text-[10px] text-white/50 leading-relaxed">
                              Khi kích hoạt hành vi này, con trỏ chuột mô phỏng sẽ xuất hiện từ góc dưới bên phải màn hình, di chuyển tự nhiên mượt mà đến vị trí của phụ đề hoặc một điểm nổi bật ngẫu nhiên trên canvas.
                            </p>
                            <p className="text-[10px] text-white/50 leading-relaxed">
                              Sau đó, chuột sẽ thao tác "vẽ" một đường vòng tròn khép kín màu đỏ bao quanh điểm đó một cách thủ công với nét vẽ imperfect (méo mó tự nhiên như chuyển động tay thật) và vẽ tiếp một mũi tên màu đỏ chỉ chuẩn xác vào vòng tròn từ một hướng ngẫu nhiên khác nhau giữa các lần xuất hiện!
                            </p>
                          </div>
                        )}
                      </div>
      ...
      {popupEditingEffect && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-[9999] flex items-center justify-center p-4">
          <div className="bg-[#0b0b14] border border-white/10 rounded-2xl w-full max-w-6xl overflow-hidden shadow-2xl flex flex-col max-h-[96vh]">
            
            {/* Header */}
            <div className="p-4 border-b border-white/5 flex items-center justify-between bg-[#12121e]">
              <div className="flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 overflow-hidden flex-1 leading-normal">
              ...
              <div className="lg:col-span-6 p-5 border-r border-white/5 overflow-y-auto space-y-4 flex flex-col bg-[#08080d]">
                ...
                <div className="bg-[#0f0f18] p-3 rounded-xl border border-white/5 space-y-3">
