    ...
    const render = () => {
      ...
      if (isPlaying) {
        ...
        if (ctxNode && ctxNode.state === 'running') {
          ...
          if ((config.enableHumanTypewriter || isTypewriterActive || config.enableHighlightDate || isTouchTypingEnabled) && subtitles && subtitles.length > 0) {
            const introTime = config.introVideoUrl ? config.introDuration : 0;
            const adjustedTime = Math.max(0, currentTime - introTime);
            const maxAudioEnd = introTime + audioDuration;
            if (currentTime >= introTime && currentTime <= maxAudioEnd) {
              ...
              if (activeBlock) {
                ...
                if (isHumanTypewriterActive || isTouchActive || isTypewriterActive || isDateHighlightActive) {
                  ...
                  const soundAllowed = isTouchActive ? (config.enableTouchTypingSound !== false) : true;
