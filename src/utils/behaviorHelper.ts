...
export function isBehaviorActiveForBlock(
  ...
  if (trimmed.startsWith('#')) {
    ...
    // Create a robust seed combining the total subtitle blocks, their texts (lengths), and the behavior key
    // This stabilizes selections across preview & render passes unless the underlying SRT content changes.
    const seed = `${total}_${behaviorKey}_${subtitles.map(s => (s.text ? s.text.length : 0)).join(',')}`;
