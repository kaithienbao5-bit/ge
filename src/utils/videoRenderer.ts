...
export function drawVideoFrame(
  ...
  if (isHandWriteActive && activeBlock) {
    ...
    if (!cached) {
      cached = {
        ...
        paperIdx: Math.floor(Math.random() * PAPERS.length),
        penIdx: Math.floor(Math.random() * PENS.length)
      };
  ...
  if (isTouchTypingActive && activeBlock) {
    ...
    const blockSeedStr = activeBlock ? (activeBlock.id + activeBlock.text + activeBlock.startTime) : "default_seed";
    let blockHash = 0;
    for (let hIndex = 0; hIndex < blockSeedStr.length; hIndex++) {
      blockHash = (blockHash << 5) - blockHash + blockSeedStr.charCodeAt(hIndex);
      blockHash |= 0;
    }
    const seedVal = Math.abs(blockHash);
  ...
  if (activeBlock && subtitles && subtitles.length > 0) {
    ...
    if (isTypewriterActive) {
      ...
      // 1. Load style preset: Always base text style on "Phong cách #1" (preset with ID 'preset_1' or index 0)
      let blockPreset: SubtitlePreset | undefined = undefined;
      if (presets && presets.length > 0) {
        blockPreset = presets.find(p => p.id === 'preset_1') || presets[0];
      }
      ...
      // 6. Select a single, consistent design template from this chosen group for the entire video to keep subtitles uniform
      const localEffectIdx = Math.abs(videoSeed * 13 + 7) % groupEffectsSelected.length;
      const selectedEffect = groupEffectsSelected[localEffectIdx];
  ...
  // Draw Circle Annotation overlay (Vẽ vòng tròn bằng chuột)
  if (isDrawCircleActive && activeBlock) {
    const scaleFactor = height / 1080;
    const blockDuration = Math.max(0.5, activeBlock.endTime - activeBlock.startTime);
    const elapsed = Math.max(0, adjustedTime - activeBlock.startTime);
    const progress = Math.max(0, Math.min(1.0, elapsed / blockDuration));

    // Seeded random for natural variations
    const rand = createSeededRandom(activeBlock.text + activeBlockIdx);
    
    // Choose a target point for the circle in the safe center area
    const targetX = width * (0.35 + rand() * 0.3);
    const targetY = height * (0.35 + rand() * 0.25);
    ...
    const noiseAmp1 = (1.5 + rand() * 3) * scaleFactor; // Slightly lower amplitude for a slightly rounder circle
    const noisePhase1 = rand() * Math.PI * 2;
    
    const noiseFreq2 = 3 + Math.floor(rand() * 4);
    const noiseAmp2 = (1.0 + rand() * 2) * scaleFactor; // Slightly lower amplitude for a slightly rounder circle
